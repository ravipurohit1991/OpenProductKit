...
| Business logic | Lives in the FastAPI app | Lives in a framework-free `core`; web/CLI are adapters |
| CLI | — | First-class control plane (`opk`) |
| Plugins | — | Entry-point plugin system + admin UI |
| Licensing | — | Entitlement abstraction with a dev stub |
...
**Choose OpenProductKit** if you want to ship the *same product* as a web app, a CLI, and (later) a desktop app, with plugins and commercial hooks, from one decoupled core.
